...
class ChatMessageSerializer(serializers.ModelSerializer):
    ...
    sender_public_key = serializers.CharField(
        source="sender.public_key", read_only=True)
    recipient_email = serializers.EmailField(
        source="recipient.email", read_only=True)
    reply_to_id = serializers.PrimaryKeyRelatedField(
        source='reply_to', read_only=True)

    class Meta:
        model = ChatMessage
        fields = ["id", "sender", "sender_email", "sender_name", "sender_public_key", "recipient",
                  "recipient_email", "message", "attachment", "attachment_type", "timestamp", "is_read",
                  "reply_to", "reply_to_id", "encrypted_symmetric_key"]
        read_only_fields = ["id", "sender", "timestamp",
                            "is_read", "sender_public_key", "reply_to_id"]


class UserConversationSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserConversation
        fields = ['id', 'pinned', 'last_read_timestamp']
